    ...
    <div className= 'flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
      ...
      <div className='flex items-center gap-4'>
       {
          ...
          ? <div className='flex items-center gap-2 cursor-pointer group relative' >
            <img className='w-8 rounded-full' ssrc={userData.image} alt=""/>
            ...
            <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
              <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4'>
                ...
                <p onClick={()=>navigate('my-reservas')} className='hover:text-black cursor-pointer' >Mis reservas</p>
                <p onClick={logout} className='hover:text-black cursor-pointer' >Cerrar Sesion</p>
              </div>
            </div>
          </div> 
      ...
      <div className={`md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all ${showMenu ? 'fixed w-full' : 'h-0 w-0'}`}>
        <div className='flex items-center justify-between px-5 py-6'>
          ...
          <img onClick={()=>setShowMenu(false)} src={assets.cross_icon} lassName='w-7' alt="" />
        </div>
        <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
        <NavLink onClick={() => setShowMenu(false)} to='/'><p className='px-4 py-2 rounded full inline-block'>HOME</p></NavLink>
            <NavLink onClick={() => setShowMenu(false)} to='/products' ><p className='px-4 py-2 rounded full inline-block'>PRODUCTOS</p></NavLink>
            <NavLink onClick={() => setShowMenu(false)} to='/about' ><p className='px-4 py-2 rounded full inline-block'>NOSOTROS</p></NavLink>
